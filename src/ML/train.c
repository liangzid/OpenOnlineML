...
/*static是静态变量，即除非程序结束否则永远都不会被释放的变量，这里指示的是当前行以及每行最大的字符数*/
...
static char* readline(FILE *input)
{
	...
	/*fgets表示从input中读取内容max_line_len个字符放入line中，因此这里是怕文件为空*/
	...
	while(strrchr(line,'\n') == NULL)
	{
		max_line_len *= 2; /*将长度乘以2*/
		line = (char *) realloc(line,max_line_len);/*调整一个指针所指向的内存块的大小，也就是将指针line的大小调整为max_line_len*/
		len = (int) strlen(line);
